import { TrendingUp, TrendingDown, Users, DollarSign, BarChart3, AlertTriangle, Activity, ShoppingCart } from "lucide-react";
import { AreaChart, Area, XAxis, YAxis, Tooltip, ResponsiveContainer, PieChart, Pie, Cell, Line } from "recharts";
...
const MentorDashboard = () => {
  ...
  const kpis = [
    ...
    { label: "Taxa Conclusão", value: `${avgCompletionRate}%`, change: "+3%", up: true, icon: BarChart3 },
    { label: "Score Saúde", value: String(businessHealthScore), change: "", up: true, icon: Activity },
  ];

  const riskData = [
    { name: "Baixo", value: state.students.filter(s => s.engagement.churnRisk === "low").length, color: "hsl(142, 70%, 45%)" },
    { name: "Médio", value: state.students.filter(s => s.engagement.churnRisk === "medium").length, color: "hsl(38, 92%, 50%)" },
    { name: "Alto", value: state.students.filter(s => s.engagement.churnRisk === "high" || s.engagement.churnRisk === "critical").length, color: "hsl(0, 72%, 51%)" },
  ];

  const revenueEngagementData = [
    { day: "01", receita: 1200, engajamento: 78 }, { day: "05", receita: 3400, engajamento: 82 },
    { day: "10", receita: 2800, engajamento: 75 }, { day: "15", receita: 5600, engajamento: avgEngagement },
    { day: "20", receita: 4200, engajamento: 88 }, { day: "25", receita: 6800, engajamento: 95 },
    { day: "30", receita: 7400, engajamento: avgEngagement },
  ];
  ...
  return (
    <div className="space-y-8 max-w-6xl">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {kpis.map((kpi, i) => (
          <div key={i} className="glass rounded-xl p-4 card-hover animate-fade-slide-in" style={{ animationDelay: `${i * 50}ms` }}>
            <div className="flex items-center justify-between mb-3">
              <kpi.icon className="h-4 w-4 text-muted-foreground" strokeWidth={1.5} />
              {kpi.change && (
                <span className={`text-[10px] font-mono font-medium flex items-center gap-0.5 ${kpi.up ? "text-success" : "text-destructive"}`}>
      ...
      <div className="glass rounded-2xl p-6">
        ...
        <div className="space-y-2">
          {state.notifications.slice(0, 8).map((item, i) => (
            <div key={item.id} className={`glass rounded-xl p-3 border-l-[3px] ${getFeedColor(item.type)} animate-fade-slide-in`} style={{ animationDelay: `${i * 40}ms` }}>
              <div className="flex items-center justify-between">
                <div className="flex-1">
                  <span className="text-xs">{item.title} {item.message}</span>
                </div>
