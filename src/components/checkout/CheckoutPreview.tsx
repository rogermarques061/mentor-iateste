...
const CheckoutPreview = ({ product, courses, previewMode }: CheckoutPreviewProps) => {
  ...
  return (
    <div
      className={`bg-background text-foreground overflow-y-auto ${isMobile ? "max-w-[375px] mx-auto rounded-3xl border border-border shadow-2xl" : ""}`}
    ...
    >
      ...
      <div className="w-full py-2 px-3 text-center text-[11px] font-semibold text-white" style={{ background: accent }}>
        <div className="flex items-center justify-center gap-1.5">
          ...
          <span className="font-mono bg-card/20 rounded px-1.5 py-0.5 text-[10px]">00:15:42</span>
        </div>
      </div>
        ...
        <div className={`grid gap-4 ${isMobile ? "grid-cols-1" : "grid-cols-2"}`}>
          ...
          <div className={`space-y-3 ${isMobile ? "order-2" : "order-1"}`}>
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[10px] font-semibold text-muted-foreground">IMPLOFY</span>
            </div>
            ...
            <div className="bg-card rounded-lg border border-border/50 p-3 ">
              <div className="flex gap-2 items-start">
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-xs font-bold text-foreground truncate">{product.name || "Novo Produto"}</div>
                  <div className="text-[9px] text-muted-foreground mt-0.5 line-clamp-2">{product.description || "Descrição do produto"}</div>
                </div>
              </div>
            ...
            <div className="bg-card rounded-lg border border-border/50 p-2.5 ">
              <div className="text-[9px] font-medium text-muted-foreground mb-1.5">Cupom de desconto</div>
              <div className="flex gap-1.5">
                <div className="flex-1 rounded px-2 py-1.5 text-[9px] bg-[#1A1A1A] border border-border text-muted-foreground/70 font-mono">
                  CÓDIGO
                </div>
            ...
            <div className="grid grid-cols-2 gap-1.5">
              {[
                ...
                <div key={i} className="flex items-center gap-1.5 bg-card rounded-lg border border-border/50 p-2 ">
                  <b.icon className="h-3 w-3 text-muted-foreground/70 shrink-0" strokeWidth={1.5} />
                  <span className="text-[9px] font-medium text-muted-foreground">{b.text}</span>
                </div>
              ))}
            </div>
            ...
            {testimonials.length > 0 && (
              <div className="space-y-1.5">
                <div className="text-[10px] font-semibold text-foreground/80">Avaliações</div>
                {testimonials.slice(0, 3).map((t, i) => (
                  <div key={i} className="bg-card rounded-lg border border-border/50 p-2.5 ">
                    ...
                    <p className="text-[8px] text-muted-foreground">"{t.text}"</p>
                  </div>
                ))}
              </div>
            )}
          ...
          <div className={isMobile ? "order-1" : "order-2"}>
            <div className="bg-card rounded-lg border border-border/50  overflow-hidden">
              ...
              <div className="flex border-b border-border/50">
                ...
                <div className="flex-1 flex items-center justify-center gap-1 py-2.5 text-[10px] font-semibold text-muted-foreground/70 border-b-2 border-transparent">
              ...
              <div className="p-3 space-y-2.5">
                <div className="text-xs font-bold text-foreground">Seus dados</div>

                {/* Form fields */}
                {["Nome completo", "E-mail", "Telefone", "CPF"].map(label => (
                  <div key={label}>
                    <div className="text-[9px] text-muted-foreground font-medium mb-1">{label}</div>
                    <div className="rounded-md px-2.5 py-2 text-[10px] bg-card border border-border text-muted-foreground/70">
                ...
                <p className="text-[7px] text-muted-foreground/70 text-center pt-1">
