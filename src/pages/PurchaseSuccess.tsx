...
import { Check, Mail, Share2, ExternalLink, Clock, Star, Copy, PartyPopper } from "lucide-react";

const purchase = {
  name: "Mentoria PATRON",
  value: "R$ 997,00",
  method: "PIX",
  orderId: "CHK-2025-001847",
      ...
      <div className="w-full max-w-lg space-y-6 relative z-10">
        ...
        <div className="glass rounded-2xl p-6 space-y-4 animate-fade-slide-in" style={{ animationDelay: "200ms" }}>
          <div className="flex items-center gap-2 text-xs text-[#34D399]">
            <Check className="h-3.5 w-3.5" /> Pedido confirmado
          </div>
          <div className="space-y-3">
            {[
              { label: "Produto", value: purchase.name },
              { label: "Valor", value: purchase.value, mono: true, gold: true },
              { label: "Método", value: purchase.method },
              { label: "Nº do pedido", value: `#${purchase.orderId}`, mono: true, primary: true },
            ].map((row, i) => (
              <div key={i} className="flex justify-between text-sm">
                <span className="text-[#9B9AA8]">{row.label}</span>
                <span className={`${row.mono ? "font-mono" : ""} ${row.gold ? "text-[#F59E0B] font-medium" : ""} ${row.primary ? "text-[#8B5CF6] text-xs" : ""}`}>
                  {row.value}
                </span>
              </div>
            ))}
          ...
          <div className="glass rounded-xl p-3 flex items-center gap-2 text-xs text-[#9B9AA8]">
            ...
            <span>Detalhes enviados para <strong className="text-[#F1F0F5]">{purchase.email}</strong></span>
          </div>
        </div>
        ...
        {/* Upsell */}
        <div
          className="glass rounded-2xl p-6 border border-[rgba(245,158,11,0.2)] space-y-4 animate-fade-slide-in"
        ...
        >
          ...
          <h3 className="font-display text-base">{upsell.name}</h3>
          <div className="flex items-baseline gap-3">
            <span className="font-mono text-2xl font-bold text-[#F59E0B]">R$ {upsell.offerPrice}</span>
            <span className="font-mono text-sm text-[#9B9AA8] line-through">R$ {upsell.originalPrice}</span>
            <span className="px-2 py-0.5 rounded-lg text-[10px] font-medium bg-[rgba(245,158,11,0.15)] text-[#F59E0B]">
              -{Math.round((1 - upsell.offerPrice / upsell.originalPrice) * 100)}%
            </span>
          </div>
          ...
          <div className="flex gap-3">
            <button className="flex-1 bg-[#F59E0B] hover:bg-[#F59E0B]/90 text-black rounded-xl py-3.5 text-sm font-semibold transition-all active:scale-[0.98]">
              Quero aproveitar
            </button>
            <button className="text-xs text-[#9B9AA8] hover:text-[#F1F0F5] transition-all px-4">
              Não, obrigado
            </button>
          </div>
        </div>
        ...
        <div className="text-center space-y-3 animate-fade-slide-in" style={{ animationDelay: "600ms" }}>
          ...
          <div className="flex justify-center gap-3">
            <button className="glass rounded-xl px-5 py-2.5 text-xs flex items-center gap-2 hover:bg-[rgba(255,255,255,0.06)] transition-all">
