  ...
  return (
    <div className="space-y-10 max-w-6xl">
      <div className="animate-fade-slide-in">
        <h1 className="font-display text-2xl">Ranking</h1>
        <p className="text-muted-foreground text-sm mt-1">Veja sua posição entre os colegas da turma.</p>
      </div>

      {/* User position card */}
      {userEntry && (
        <div className="glass rounded-2xl p-6 border-l-2 border-primary animate-fade-slide-in" style={{ animationDelay: "80ms" }}>
          <div className="flex items-center gap-4">
            <span className="font-mono text-3xl font-bold text-primary">{userEntry.pos}º</span>
      ...
      {/* Podium */}
      <div className="flex items-end justify-center gap-4 animate-fade-slide-in" style={{ animationDelay: "160ms" }}>
        {podiumOrder.map((idx) => {
          const r = top3[idx];
          const heights = { 0: "h-32", 1: "h-24", 2: "h-20" };
          return (
            <div key={r.pos} className="flex flex-col items-center gap-2 w-28">
              ...
              <p className="text-sm font-medium text-center truncate w-full">{r.name}</p>
